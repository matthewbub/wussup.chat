...
describe('Admin Endpoints', () => {
	...
	describe('Promote User', () => {
		it('should promote a regular user to admin', async () => {
			...
			const promoteResponse = await fetch(`${API_URL}/v3/admin/users/${regularUserData.id}/promote`, {
				method: 'POST',
				headers: {
					Authorization: `Bearer ${adminAccessToken}`,
				},
	...
	describe('Suspend User', () => {
		it('should suspend a user account', async () => {
			const { accessToken: adminAccessToken } = await createAdminUser();
			const { accessToken: regularUserAccessToken, userData: regularUserData } = await getRegularUser();

			// Suspend the user
			const suspendResponse = await fetch(`${API_URL}/v3/admin/users/${regularUserData.id}/suspend`, {
				method: 'POST',
				headers: {
					Authorization: `Bearer ${adminAccessToken}`,
				},
