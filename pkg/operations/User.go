...
func GetUserFromDatabase(username string) (*utils.UserObject, error) {
	...
	stmt, err := db.Prepare("SELECT id, username, email, security_questions_answered FROM users WHERE username = ?")
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func GetUserByID(userID string) (*utils.UserObject, error) {
	...
	user := utils.UserObject{}
	stmt, err := db.Prepare("SELECT id, username, email, security_questions_answered FROM users WHERE id = ?")
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func GetUserWithPasswordByID(userID string) (*utils.UserWithPassword, error) {
	...
	stmt, err := db.Prepare("SELECT id, username, email, security_questions_answered, password FROM users WHERE id = ?")
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func GetUserWithRoleByID(userID string) (*utils.UserWithRole, error) {
	...
	stmt, err := db.Prepare("SELECT id, username, email, security_questions_answered, application_environment_role, password FROM users WHERE id = ?")
	if err != nil {
		log.Println(err)
		return nil, err
	}
